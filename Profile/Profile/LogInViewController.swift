...
class LogInViewController: UIViewController,UITextFieldDelegate,UserService {
...
//    }
    ...
    @objc func buttonPressed(_ sender: UIButton) {
        if let enteredLogin = login.text, let enteredPassword = password.text {

            if let authenticatedUser = getUser(withLogin: enteredLogin, password: enteredPassword)

            {
                print("User authenticated: \(authenticatedUser.fullName)")
                let profileViewController = ProfileViewController()
                profileViewController.currentUser = authenticatedUser
                navigationController?.pushViewController(profileViewController, animated: true)
            } else {
                print("Authentication failed")
                let alert = UIAlertController(title: "Error", message: "Incorrect login data", preferredStyle: .alert)
