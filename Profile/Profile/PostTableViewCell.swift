...
let label = UILabel()
    ...
    func applyFilter(_ filter: ColorFilter) {
            guard let originalImage = videoImageView.image else { return }

            ImageProcessor().processImage(sourceImage: originalImage, filter: filter) { [weak self] filteredImage in
                DispatchQueue.main.async {
                    self?.videoImageView.image = filteredImage
                }
            }
        }
